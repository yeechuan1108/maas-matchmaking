...
import pandas as pd  # ── 🏗️ New: Handling interactive data tables ──
...
for key, default in [
    ...
    ("messages", []),
    ("last_user_filters", {}),  
    ("last_debug_log", []),                                # ── 🏗️ New: Save filter logs ──
    ("last_llm_output", {"thinking": "", "full": ""}),     # ── 🏗️ New: Save LLM output ──
]:
...
combined_intent_filter_prompt = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "3. If the user asks a general question without specific constraints (e.g., 'list all manufacturing services'), return ALL fields as null except 'action'.\n\n"
...
# ── 🏗️ New: Sidebar Control Center ──
with st.sidebar:
    st.header("⚙️ Control Center")
    
    # 1. Developer Mode Switch (Binds the Session State key to ensure state persistence)
    st.toggle("Turn on Developer Mode", value=False, key="dev_mode")
    st.markdown("---")
    
    # 2. A decision-maker-friendly summary panel for filter statuses
    st.subheader("🔍 Currently Applied Filter Conditions")
    f = st.session_state.get("last_user_filters", {})
    if f:
        if f.get("target_material"): st.write(f"📦 Target Material: {f['target_material']}")
        if f.get("excluded_material"): st.write(f"🚫 Excluded Material: {f['excluded_material']}")
        if f.get("target_cert"): st.write(f"✅ Required Certificate: {f['target_cert']}")
        if f.get("excluded_cert"): st.write(f"❌ Excluded Certificate: {f['excluded_cert']}")
        
        # Delivery Logic
        min_dt, max_dt = f.get("min_delivery_days"), f.get("max_delivery_days")
        if min_dt is not None and max_dt is not None:
            if min_dt == max_dt: st.write(f"🚚 Delivery Time: Exactly {int(min_dt)} days")
            else: st.write(f"🚚 Delivery Time: {int(min_dt)} to {int(max_dt)} days")
        elif max_dt is not None: st.write(f"🚚 Delivery Time Limit: ≤ {int(max_dt)} days")
        elif min_dt is not None: st.write(f"🚚 Delivery Time Limit: ≥ {int(min_dt)} days")

        # GHG Logic
        min_ghg, max_ghg = f.get("min_ghg"), f.get("max_ghg")
        if min_ghg is not None and max_ghg is not None:
            if min_ghg == max_ghg: st.write(f"🌱 GHG Emission: Exactly {int(min_ghg)} kg")
            else: st.write(f"🌱 GHG Emission: {int(min_ghg)} to {int(max_ghg)} kg")
        elif max_ghg is not None: st.write(f"🌱 GHG Emission Limit: ≤ {int(max_ghg)} kg")
        elif min_ghg is not None: st.write(f"🌱 GHG Emission Limit: ≥ {int(min_ghg)} kg")

        # Yield Logic
        min_y, max_y = f.get("min_yield"), f.get("max_yield")
        if min_y is not None and max_y is not None:
            if min_y == max_y: st.write(f"📈 Yield Rate: Exactly {int(min_y)}%")
            else: st.write(f"📈 Yield Rate: {int(min_y)}% to {int(max_y)}%")
        elif min_y is not None: st.write(f"📈 Yield Rate Limit: ≥ {int(min_y)}%")
        elif max_y is not None: st.write(f"📈 Yield Rate Limit: ≤ {int(max_y)}%")

        # Precision Logic
        min_p, max_p = f.get("min_precision"), f.get("max_precision")
        if min_p is not None and max_p is not None:
            if min_p == max_p: st.write(f"🎯 Precision Requirement: Exactly IT{int(min_p)}")
            else: st.write(f"🎯 Precision Requirement: IT{int(min_p)} to IT{int(max_p)}")
        elif max_p is not None: st.write(f"🎯 Precision Requirement: IT{int(max_p)} or Better")
        elif min_p is not None: st.write(f"🎯 Precision Requirement: IT{int(min_p)} or Worse")
    else:
        st.caption("No filter conditions set")
        
    st.markdown("---")
    
    # 3. Hide the original schema
    with st.expander("View Ontology Schema"):
...
# ── 🏗️ Interactive Data Table Rendering Functions ──
def render_scoring_table(report_data):
    if not report_data:
        st.info("_No services matched your criteria._")
        return

    # Convert the dictionary to a DataFrame and insert a “Rank” column
    df = pd.DataFrame(report_data)
    df.insert(0, "Rank", range(1, len(df) + 1))
    
    # Use `st.dataframe` for minimal formatting, retaining `NumberColumn` to ensure correct sorting
    st.dataframe(
        df,
        use_container_width=True,
        hide_index=True,
        column_order=["Rank", "Service", "Total_Score", "DeliveryTime", "GHG", "Material", "Precision", "Yield", "Quality_Cert", "Energy_Cert"],
        column_config={
            "Rank": st.column_config.NumberColumn("Rank", format="%d"),
            "Service": st.column_config.TextColumn("Service"),
            "Total_Score": st.column_config.NumberColumn("Score", format="%.1f"),
            "DeliveryTime": st.column_config.NumberColumn("Delivery (Day)"),
            "GHG": st.column_config.NumberColumn("GHG (kg)"),
            "Material": st.column_config.TextColumn("Material"),
            "Precision": st.column_config.NumberColumn("Precision (IT)"),
            "Yield": st.column_config.NumberColumn("Yield (%)"),
            "Quality_Cert": st.column_config.TextColumn("Quality Cert"),
            "Energy_Cert": st.column_config.TextColumn("Energy Cert"),
        }
    )

# ── 🛠️ Enable the history log to display an interactive DataFrame ──
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        if message.get("is_new_search"):
            st.markdown("### 📊 Weighted Scoring Table")
            render_scoring_table(message.get("table_data", []))
...
if prompt := st.chat_input("What kind of manufacturing services are you looking for?"):
    ...
    with st.chat_message("assistant"):
        ...
        if python_forced_followup:
            action = "FOLLOW_UP"
            # ── 🛠️ Modification 2: Securely hide Python route hints ──
            if st.session_state.get("dev_mode", False):
            ...
            with st.spinner("🧠 Understanding Requirements (1/2)..."):
                ...
                # 1. Ensure that post-processing protection and correction (including exact match filling) are performed first
                ...
                if parsed is None:
                    ...
                    # 2. Create the final, clean `user_filters`
                    ...
                    # 3. Ensure the filters are correct and complete before saving to Session State for sidebar display!
                    if action == "NEW_SEARCH":
                        st.session_state["last_user_filters"] = user_filters

        # ── 🛠️ Modification 3: Securely hide execution mode ──
        if st.session_state.get("dev_mode", False):
            st.caption(f"🤖 Execution Mode: **[{action}]**")

        # Call Info Prompt
        # ── 🛠️ Note: Precision tooltips are only displayed in Developer Mode. ──
        if st.session_state.get("dev_mode", False):
        ...
        if action == "NEW_SEARCH":
            with st.spinner("📡 Search the knowledge base（2/2）..."):
                ...
                # ── 🛠️ Modification: Log entries are only saved to the state; they are not rendered here. ──
                if action == "NEW_SEARCH":
                    st.session_state["last_debug_log"] = debug_filter_log
        ...
        if final_context_data:
            ...
            # ── 🛠️ Call the new function to render the current interactive form ──
            if action == "NEW_SEARCH":
                st.markdown("### 📊 Weighted Scoring Table")
                render_scoring_table(final_context_data.get("Processed_Report_Raw", []))
                st.markdown("### 💬 Interpretation")
            ...
            # ── 🛠️ Modification: Only store the LLM output in state; do not render it here. ──
            st.session_state["last_llm_output"] = {
                "thinking": thinking_content,
                "full": full_llm_output
            }
            ...
            # ── 🛠️ 將 table_data 存入 session_state 供歷史紀錄使用 ──
            if action == "NEW_SEARCH":
                st.session_state.messages.append({
                    "role": "assistant", 
                    "content": clean_answer,
                    "is_new_search": True,
                    "table_data": final_context_data.get("Processed_Report_Raw", [])
                })
            else:
                st.session_state.messages.append({
                    "role": "assistant", 
...
# ──────────────────────────────────────────────
# ── 🏗️ Unified Debug Panel Rendering ──
# ──────────────────────────────────────────────
if st.session_state.get("dev_mode", False):
    with st.sidebar:
        st.markdown("---")
        st.subheader("🛠️ Developer Logs")
        
        # 1. JSON Filter Criteria and Accuracy Diagnosis
        f = st.session_state.get("last_user_filters", {})
        if f:
            with st.expander("🔍 [DEBUG] Detected Filter Conditions JSON"):
                st.json(f)
                if "min_precision" in f or "max_precision" in f:
                    min_it = f.get("min_precision")
                    max_it = f.get("max_precision")
                    st.markdown("**🎯 Precision Extraction Diagnosis: **")
                    if min_it: st.text(f"  min_precision = {min_it}\n  → Meaning: Precision lower than IT{min_it}")
                    if max_it: st.text(f"  max_precision = {max_it}\n  → Meaning: Precision higher than IT{max_it}")
                    if min_it and max_it:
                        if float(min_it) > float(max_it): st.warning(f"  ⚠️ Mathematically Contradictory: min ({min_it}) > max ({max_it})")
                        else: st.success(f"  ✅ Logically Consistent: min ({min_it}) ≤ max ({max_it})")

        # 2. Supplier Filtering Logs
        log = st.session_state.get("last_debug_log", [])
        if log:
            with st.expander("🕵️ [DEBUG] Filter Diagnosis Log"):
                for l in log: st.text(l)

        # 3. LLM Complete Thinking and Output
        llm_out = st.session_state.get("last_llm_output", {})
        if llm_out.get("full"):
            with st.expander("🐛 [DEBUG] LLM Complete Output"):
                if llm_out.get("thinking"):
                    st.markdown("**🧠 Thinking Process:**")
                    st.code(llm_out["thinking"], language="text")
                st.markdown("**📄 Full Output:**")
                st.code(llm_out["full"], language="text")